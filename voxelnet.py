...
class VisibilityFeatureExtractor(nn.Module):
    def __init__(self,
        ...
        self.x_size = int((px_max - px_min) / self.voxel_size)
        self.y_size = int((py_max - py_min) / self.voxel_size)
        self.z_size = int((pz_max - pz_min) / self.voxel_size)
        ...
        self.filter = nn.Conv2d(in_channels=self.z_size,
    ...
    def forward(self, logodds):
        ...
        occupancy = torch.sigmoid(torch.from_numpy(logodds))
        occupancy = occupancy.type(torch.FloatTensor)
        ...
        # print('visibility shape:', features.shape)
...
class VoxelNet(nn.Module):
    def __init__(self,
        ...
        """
        """
        self.visibility_feature_extractor = VisibilityFeatureExtractor(voxel_size, pc_range, 32)
        """
        """
    ...
    def forward(self, example):
        ...
        """
        """
        logodds = None if "logodds" not in example else example["logodds"]
        """
        """
        ...
        if self._use_sparse_rpn:
            ...
            """
            """
            spatial_features = torch.cat((spatial_features, visibility_features), dim=1)

    ...
    const bool culling_original)
{
    ...
    for (int t = 0; t < T; ++ t) {
        ...
        for (const int & i : original_indices[t]) {
            ...
            bool truncated =
            _voxel_traversal(visited_voxels, origin, point, grid_size, voxel_size);
...
std::tuple<Eigen::VectorXf, Eigen::VectorXb, Eigen::VectorXb> _compute_logodds_and_masks_no_timestamp(
        const Eigen::MatrixXf & original_points,
        const Eigen::MatrixXf & sampled_points,
        const Eigen::RowVector3f & sensor_origins,
        //const Eigen::VectorXf & time_stamps,
        const Eigen::VectorXf & pc_range,
        const double voxel_size,
        const double lo_occupied,
...
//    }
    ...
    //for (int t = 0; t < T; ++ t) {
        // set up update variable and sensor origin
        Eigen::VectorXf update = Eigen::VectorXf::Zero(G3);
        Eigen::Vector3d origin = (sensor_origins - offset3d).cast<double>();
...
Eigen::VectorXf _compute_logodds_no_timestamp(const Eigen::MatrixXf & original_points,
                                 const Eigen::RowVector3f & sensor_origins,
                                 //const Eigen::VectorXf & time_stamps,
                                 const Eigen::VectorXf & pc_range,
                                 const double voxel_size,
...
PYBIND11_MODULE(mapping, m) {
    ...
    m.def("compute_logodds_and_masks_no_timestamp",
          &_compute_logodds_and_masks_no_timestamp,
          py::arg("original_points"),
          py::arg("sampled_points"),
          py::arg("sensor_origins"),
          //py::arg("time_stamps"),
    ...
    m.def("compute_logodds_no_timestamp",
          &_compute_logodds_no_timestamp,
          py::arg("original_points"),
          py::arg("sensor_origins"),
          //py::arg("time_stamps"),
          py::arg("pc_range"),

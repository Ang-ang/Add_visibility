from second.data.mapping import mapping
...
def prep_pointcloud(input_dict,
    ...
    # print("before train", points.shape)
    if training:
        # print(gt_names)
        ...
        if db_sampler is not None:
            ...
            # print("after sample", points.shape)
        ...
        prep.noise_per_object_v3_(
            ...
            group_ids=group_ids,
            num_try=100)
        # print("remove noise", points.shape)
        ...
        # print("after global translation", points.shape)
    ...
    if len(points) > num_original_points:
        original_points = points[:num_original_points]
        sampled_points = points[num_original_points:]
        logodds, original_mask, sampled_mask = mapping.compute_logodds_and_masks_no_timestamp(original_points,
                                                                                              sampled_points,
                                                                                              sensor_origins, pc_range,
                                                                                              min(voxel_size))
        # print(logodds, original_mask.shape, sampled_mask.shape)
        # points = np.concatenate((original_points[original_mask], sampled_points[sampled_mask]))
    else:
        original_points = points
        logodds = mapping.compute_logodds_no_timestamp(original_points, sensor_origins, pc_range, min(voxel_size))

    # print(points.dtype)  # float32
    # print(logodds.shape)  # (1321344,)
